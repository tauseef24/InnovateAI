<template>
  <div class="space-y-4 p-4 flex flex-col h-full bg-white shadow-sm border-r">
    <div class="px-3 py-2 flex-1">
      <NuxtLink class="flex items-center pl-3 mb-14" to="/dashboard">
        <div class="relative h-8 w-8 mr-4">
          <img src="/logo.svg" />
        </div>
        <h1 class="text-2xl font-bold">InnovateAI</h1>
      </NuxtLink>
      <div class="space-y-1">
        <NuxtLink
          ...
          :key="route.link"
          :to="route.link"
          class="text-sm group text-primary flex p-3 w-full justify-start font-medium cursor-pointer rounded-lg hover:transition hover:bg-muted"
          :class="{
            'bg-muted': currentRoute.path === route.link,
          }"
        >
          <div class="flex flex-1 items-center capitalize">
